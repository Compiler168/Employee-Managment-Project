...
import os  # Import os moduleimport psycopg2
from tkinter import *
...
class Employee:
    def __init__(self, root):
        ...
        btn_Delete=Button(Button_frame,text="Delete",bg='#F44336',fg='white', font=('Arial', 10, 'bold'), width=13, )
...
# Show All Button
        ...
        self.Employee_table=ttk.Treeview(Table_frame,column=("Dep","Name","Desi","Email","Married Status","Employee ID","DOB","DOJ","Gender","Phone No","Country","Salary",),xscrollcommand=scroll_x.set ,yscrollcommand=scroll_y.set )
...
# ----------Function Declaration-----
    def add_data(self):
       if self.var_Dep.get() == "" or self.var_Email.get() == "":
        ...
        try:
            ...
            my_cursor.execute(
                    'INSERT INTO employee (Department, Name, Designation, Email, Married Status, Employee iD, DOB, DOJ, Gender, Phone No, Country, Salary) '
    ...
    def fetch_data(self):
            ...
            my_cursor = conn.cursor()
            my_cursor.execute('select *employee')
        ...
        if self.var_Dep.get() == "" or self.var_Email.get() == "":
            ...
            try:
                ...
                if Update:
                    ...
                    my_cursor = conn.cursor()
                    my_cursor.execute(
                        'UPDATE employee SET Department=%s, Name=%s, Designation=%s, Email=%s, `Married Status`=%s, DOB=%s, DOJ=%s, Gender=%s, `Phone No`=%s, Country=%s, Salary=%s '
                        'WHERE `Employee ID`=%s',
                        (
                            self.var_Dep.get(),
                            self.var_Name.get(),
    ...
    def add_data(self):
       if self.var_Dep.get() == "" or self.var_Email.get() == "":
        ...
        try:
            ...
            # Insert data into the employee table
            insert_query = '''INSERT INTO Final (department, name, designation, email, married_status, dob, doj, gender, phone, country, salary)
        ...
        if self.var_Dep.get() == "" or self.var_Email.get() == "":
            ...
            try:
                ...
                if Update:
                    ...
                    my_cursor = conn.cursor()
                    my_cursor.execute(
                        'UPDATE Final SET department=%s, name=%s, designation=%s, email=%s, married_Status =%s, dob=%s, doj=%s, gender=%s, phone_no=%s, country=%s, salary=%s '
                        'WHERE `employee id`=%s',
                        (
                            self.var_Dep.get(),
                            self.var_Name.get(),
        ...
        if self.var_EmployeeID.get() == "":
            ...
            try:
                ...
                if Update:
                    ...
                    my_cursor = conn.cursor()
                    my_cursor.execute(
                        'UPDATE employee SET department=%s, name=%s, designation=%s, email=%s, married_status=%s, dob=%s, doj=%s, gender=%s, phone_no=%s, country=%s, salary=%s '
                        'WHERE `employee id`=%s',
                        (
                            self.var_Dep.get(),
                            self.var_Name.get(),
